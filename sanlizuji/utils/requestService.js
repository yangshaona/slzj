  ...
  index: {
    getGoodsList: function (page, cb) {
      ...
      wx.request({
        ...
        url: app.globalData.url+'QueryList&page=' + page+'&perpage=5',
        method: 'GET',
        ...
        success: (res) => {
          ...
          for (let i = 0; i < data.length; i++) {
            goods.push({
              id: data[i].id,
              goods_img: data[i].goods_titlepage,
              goods_title: data[i].goods_name,
              goods_desc: data[i].goods_desc,
              tea_tag: data[i].goods_tag,
              tea_price: data[i].goods_price,
            })
          };
          wx.request({
            header: utils.requestHeader(),
            url: app.globalData.url + 'GetTopSwiper',
